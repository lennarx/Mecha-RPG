...
- **Métrica de distancia: Manhattan**, tanto para movimiento (BFS en 4 direcciones) como para rango de ataque. `AStarGrid2D` corre en `DiagonalMode.Never` para que el pathfinding coincida con esa métrica — sin esto, un objetivo en diagonal quedaba a rango de ataque pero al doble de distancia de movimiento.
- **Recurso de tensión: CALOR.** Escala 0-10 (`CombatConstants.MaxTension`). Se acumula al disparar (`WeaponData.TensionCost`, 3 en el arma de entrenamiento) y al moverse (`CombatConstants.MoveHeatCost` = 1), obligando a elegir entre disparar fuerte o desplazarse. Este es el elemento que separa el juego de un táctico genérico.
  ...
  **Disipación:** al final de un turno en el que la unidad usó alguna acción, disipa `CombatConstants.HeatDissipationPerTurn` (2); si no usó ninguna acción -- por elección propia para enfriarse, o porque la sobrecarga se lo impidió -- disipa el doble, `CombatConstants.NoActionHeatDissipation` (4). Enfriarse a propósito es una jugada táctica válida.
